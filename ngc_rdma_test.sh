...
readarray -t SERVER_CORES_ARR < <(ssh "${SERVER_IP}" -l root numactl -H | grep -i "node ${SERVER_NUMA_NODE} cpus" | awk '{print substr($0,14)}')
readarray -t CLIENT_CORES_ARR < <(ssh "${CLIENT_IP}" -l root numactl -H | grep -i "node ${CLIENT_NUMA_NODE} cpus" | awk '{print substr($0,14)}')
