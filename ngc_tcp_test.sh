...
CLIENT_CORE_USAGES_FILE="/tmp/ngc_client_core_usages.txt"
SERVER_CORE_USAGES_FILE="/tmp/ngc_server_core_usages.txt"
CLIENT_ALL_CORE_USAGES_FILE="/tmp/ngc_all_client_core_usages.txt"
SERVER_ALL_CORE_USAGES_FILE="/tmp/ngc_all_server_core_usages.txt"
SERVER_RESULT_RUN="/tmp/ngc_server_result_run.txt"
CLIENT_RESULT_RUN="/tmp/ngc_client_result_run.txt"
RESULT_FILE="/tmp/ngc_result_run.txt"
RESULT_SUMMARY="/tmp/ngc_summary_result_run.txt"
...
if [ $LINK_TYPE -eq 1 ]; then
	   ...
	   #CLIENT_NUM_CHANNELS=`echo $(($CLIENT_CPUCOUNT<$CLIENT_PRESET_MAX ? $CLIENT_CPUCOUNT : $CLIENT_PRESET_MAX))`
       #ssh ${CLIENT_TRUSTED} 'for f in `seq 0 '"$((CLIENT_NUM_CHANNELS-1))"'`; do echo 32768 > /sys/class/net/'"$CLIENT_NETDEV"'/queues/rx-$f/rps_flow_cnt; done'
	   ...
	   #SERVER_NUM_CHANNELS=`echo $(($SERVER_CPUCOUNT<$SERVER_PRESET_MAX ? $SERVER_CPUCOUNT : $SERVER_PRESET_MAX))`
       #ssh ${SERVER_TRUSTED} 'for f in `seq 0 '"$((SERVER_NUM_CHANNELS-1))"'`; do echo 32768 > /sys/class/net/'"${SERVER_NETDEV}"'/queues/rx-$f/rps_flow_cnt; done'
fi
...
for node in $(seq $CLIENT_BASE_NUMA $((CLIENT_BASE_NUMA+CLIENT_LOGICAL_NUMA_PER_SOCKET-1))) ; do
...
for node in $(seq $SERVER_BASE_NUMA $((SERVER_BASE_NUMA+SERVER_LOGICAL_NUMA_PER_SOCKET-1))) ; do
...
echo "Client core info: " 
echo "CLIENT_PHYSICAL_CORE_COUNT=$CLIENT_PHYSICAL_CORE_COUNT"
echo "CLIENT_PHYSICAL_CORES=${CLIENT_PHYSICAL_CORES[@]}"
echo "CLIENTS_AFFINITY_CORES=${CLIENTS_AFFINITY_CORES[@]}"
echo "CLIENT_LOGICAL_CORES=${CLIENT_LOGICAL_CORES[@]}"

echo "Server core info: " 
echo "SERVER_PHYSICAL_CORE_COUNT=$SERVER_PHYSICAL_CORE_COUNT"
echo "SERVER_PHYSICAL_CORES=${SERVER_PHYSICAL_CORES[@]}"
echo "SERVER_AFFINITY_CORES=${SERVER_AFFINITY_CORES[@]}"
echo "SERVER_LOGICAL_CORES=${SERVER_LOGICAL_CORES[@]}"
