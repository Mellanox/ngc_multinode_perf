...
scriptdir="$(dirname "$0")"
bash ${scriptdir}/ipsec_full_offload_setup.sh ${CLIENT_TRUSTED} ${CLIENT_DEVICE} ${SERVER_TRUSTED} ${SERVER_DEVICE} ${LOCAL_BF} ${REMOTE_BF} $(( ${MTU_SIZE} + 500 ))

# Run tcp test
bash ${scriptdir}/ngc_tcp_test.sh ${CLIENT_TRUSTED} ${CLIENT_DEVICE} ${SERVER_TRUSTED} ${SERVER_DEVICE} HALF DONT_CHANGE
